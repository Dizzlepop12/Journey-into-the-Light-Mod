...
public class EntityBouncingProjectile extends EntityThrowable {
	...
	public void onUpdate() {
		...
		super.onUpdate();
		for(int i = 0; i < 6; ++i) {
			Particle effect = new EntityBouncingFX(this.worldObj, this.posX, this.posY - 1.0F, this.posZ, 0.0D, 0.0D, 0.0D);
	...
	protected void onImpact(RayTraceResult par1) {
		...
		if(this.bounces == maxBounces) this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + (double)f, this.posY + 0.0D + (double)f1, this.posZ + (double)f2, 0.0D, 0.0D, 0.0D, new int[0]);
		else this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (double)f, this.posY + 0.0D + (double)f1, this.posZ + (double)f2, 0.0D, 0.0D, 0.0D, new int[0]);
	}
}