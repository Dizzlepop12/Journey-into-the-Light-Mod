...
public abstract class EntityEssenceArrow extends EntityArrow implements IProjectile {
	...
	private static final DataParameter<Byte> CRITICAL = EntityDataManager.<Byte>createKey(EntityEssenceArrow.class, DataSerializers.BYTE);
	private int xTile;
	private int yTile;
	private int zTile;
	...
	public EntityEssenceArrow.PickupStatus pickupStatus;
	...
	public EntityEssenceArrow(World worldIn) {
		...
		this.yTile = -1;
		this.zTile = -1;
		this.pickupStatus = EntityEssenceArrow.PickupStatus.DISALLOWED;
		this.damage = 2.0D;
		this.setSize(0.5F, 0.5F);
	}

	public EntityEssenceArrow(World worldIn, double d, double d1, double d2) {
		this(worldIn);
		this.setPosition(d, d1, d2);
	}

	public EntityEssenceArrow(World worldIn, EntityLivingBase e) {
		this(worldIn, e.posX, e.posY + (double)e.getEyeHeight() - 0.10000000149011612D, e.posZ);
		this.shootingEntity = e;

		if (e instanceof EntityPlayer) {
			this.pickupStatus = EntityEssenceArrow.PickupStatus.ALLOWED;
		}
	}
		...
		if (entity != null) {
			...
			if (entity.attackEntityFrom(damagesource, (float)i)) {
				...
				if (!this.worldObj.isRemote && this.motionX * this.motionX + this.motionY * this.motionY + this.motionZ * this.motionZ < 0.0010000000474974513D) {
					if (this.pickupStatus == EntityEssenceArrow.PickupStatus.ALLOWED) {
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("pickup", 99)) {
			this.pickupStatus = EntityEssenceArrow.PickupStatus.getByOrdinal(compound.getByte("pickup"));
		}
		else if (compound.hasKey("player", 99)) {
			this.pickupStatus = compound.getBoolean("player") ? EntityEssenceArrow.PickupStatus.ALLOWED : EntityEssenceArrow.PickupStatus.DISALLOWED;
		}
	}
	...
	public void onCollideWithPlayer(EntityPlayer entityIn) {
		if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == EntityEssenceArrow.PickupStatus.ALLOWED || this.pickupStatus == EntityEssenceArrow.PickupStatus.CREATIVE_ONLY && entityIn.capabilities.isCreativeMode;

			if (this.pickupStatus == EntityEssenceArrow.PickupStatus.ALLOWED && !entityIn.inventory.addItemStackToInventory(this.getArrowStack())) {
				flag = false;
			}
	...
	public static enum PickupStatus {
		...
		public static EntityEssenceArrow.PickupStatus getByOrdinal(int ordinal) {
