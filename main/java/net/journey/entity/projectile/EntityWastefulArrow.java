...
public abstract class EntityWastefulArrow extends EntityArrow implements IProjectile {
	...
	private static final DataParameter<Byte> CRITICAL = EntityDataManager.<Byte>createKey(EntityWastefulArrow.class, DataSerializers.BYTE);
	private int xTile;
	private int yTile;
	private int zTile;
	...
	public EntityWastefulArrow.PickupStatus pickupStatus;
	...
	public EntityWastefulArrow(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
		this.pickupStatus = EntityWastefulArrow.PickupStatus.DISALLOWED;
		this.damage = 2.0D;
		this.setSize(0.5F, 0.5F);
	}

	public EntityWastefulArrow(World worldIn, double d, double d1, double d2) {
		this(worldIn);
		this.setPosition(d, d1, d2);
	}

	public EntityWastefulArrow(World worldIn, EntityLivingBase e) {
		this(worldIn, e.posX, e.posY + (double)e.getEyeHeight() - 0.10000000149011612D, e.posZ);
		this.shootingEntity = e;

		if (e instanceof EntityPlayer) {
			this.pickupStatus = EntityWastefulArrow.PickupStatus.ALLOWED;
		}
	}
	...
	@Override
	public void setAim(Entity entity, float p2, float p3, float p4, float p5, float p6) {
		float f = -MathHelper.sin(p3 * 0.017453292F) * MathHelper.cos(p2 * 0.017453292F);
		float f1 = -MathHelper.sin(p2 * 0.017453292F);
		float f2 = MathHelper.cos(p3 * 0.017453292F) * MathHelper.cos(p2 * 0.017453292F);
		this.setThrowableHeading((double)f, (double)f1, (double)f2, p5, p6);
		...
		if (entity != null) {
			...
			if (entity.attackEntityFrom(damagesource, (float)i)) {
				...
				if (!this.worldObj.isRemote && this.motionX * this.motionX + this.motionY * this.motionY + this.motionZ * this.motionZ < 0.0010000000474974513D) {
					if (this.pickupStatus == EntityWastefulArrow.PickupStatus.ALLOWED) {
	...
	public static void dataFixer1(DataFixer fix, String s){}

	public static void dataFixer1(DataFixer fix) {
		func_189657_a(fix, "Arrow");
	}
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("pickup", 99)) {
			this.pickupStatus = EntityWastefulArrow.PickupStatus.getByOrdinal(compound.getByte("pickup"));
		}
		else if (compound.hasKey("player", 99)) {
			this.pickupStatus = compound.getBoolean("player") ? EntityWastefulArrow.PickupStatus.ALLOWED : EntityWastefulArrow.PickupStatus.DISALLOWED;
		}
	}
	...
	public void onCollideWithPlayer(EntityPlayer entityIn) {
		if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == EntityWastefulArrow.PickupStatus.ALLOWED || this.pickupStatus == EntityWastefulArrow.PickupStatus.CREATIVE_ONLY && entityIn.capabilities.isCreativeMode;

			if (this.pickupStatus == EntityWastefulArrow.PickupStatus.ALLOWED && !entityIn.inventory.addItemStackToInventory(this.getArrowStack())) {
				flag = false;
			}
	...
	public static enum PickupStatus {
		...
		public static EntityWastefulArrow.PickupStatus getByOrdinal(int ordinal) {
