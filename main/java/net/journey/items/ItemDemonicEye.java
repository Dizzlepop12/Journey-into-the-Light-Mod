...
public class ItemDemonicEye extends ItemMod {
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (playerIn.canPlayerEdit(pos.offset(side), side, stack) && iblockstate.getBlock() == JourneyBlocks.corbaPortalFrame && !((Boolean)iblockstate.getValue(BlockCorbaPortalFrame.EYE)).booleanValue()) {
			if (worldIn.isRemote) {
				...
				EnumFacing enumfacing1 = (EnumFacing)iblockstate.getValue(BlockCorbaPortalFrame.FACING);
				int l = 0;
				int j = 0;
				boolean flag1 = false;
				boolean flag = true;
				...
				for (int k = -2; k <= 2; ++k) {
					...
					if (iblockstate1.getBlock() == JourneyBlocks.corbaPortalFrame) {
						if (!((Boolean)iblockstate1.getValue(BlockCorbaPortalFrame.EYE)).booleanValue()) {
							flag = false;
							break;
						}
				...
				if (flag && j == l + 2) {
					...
					for (i1 = l; i1 <= j; ++i1) {
						...
						if (iblockstate3.getBlock() != JourneyBlocks.corbaPortalFrame || !((Boolean)iblockstate3.getValue(BlockCorbaPortalFrame.EYE)).booleanValue()) {
							flag = false;
							break;
						}
					}
					...
					for (i1 = l - 1; i1 <= j + 1; i1 += 4) {
						...
						for (j1 = 1; j1 <= 3; ++j1) {
							...
							if (iblockstate2.getBlock() != JourneyBlocks.corbaPortalFrame || !((Boolean)iblockstate2.getValue(BlockCorbaPortalFrame.EYE)).booleanValue()) {
								flag = false;
								break;
							}
						}
					...
					if (flag) {
						for (i1 = l; i1 <= j; ++i1) {
							...
							for (j1 = 1; j1 <= 3; ++j1) {
								blockpos4 = blockpos1.offset(enumfacing1, j1);
								//TODO worldIn.setBlockState(blockpos4, JourneyBlocks.corbaPortal.getDefaultState(), 2);
							}
						}
					}
				}
