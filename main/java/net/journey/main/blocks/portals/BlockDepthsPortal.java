...
public class BlockDepthsPortal extends BlockBreakable {
	...
	public static Block portal = JourneyBlocks.depthsPortal;
	public static Block portalFrame = JourneyBlocks.depthsPortalFrame;
	public static Block fire = JourneyBlocks.essenceFire;
	public int dimensionID = Config.depths;
	
	public BlockDepthsPortal(String name, String finalName) {
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		BlockDepthsPortal.Size blockportal$size = new BlockDepthsPortal.Size(worldIn, pos, EnumFacing.Axis.X);
		if(blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			blockportal$size.placePortalBlocks();
			return true;
		} else {
			BlockDepthsPortal.Size blockportal$size1 = new BlockDepthsPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
		...
		BlockDepthsPortal.Size blockportal$size = new BlockDepthsPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
		LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

		if(!blockportal$size.isValid()) {
			enumfacing$axis = EnumFacing.Axis.X;
			blockportal$size = new BlockDepthsPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
		}
		...
		public void placePortalBlocks() {
			for(int i = 0; i < this.width; i++) {
				...
				for(int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), portal.getDefaultState().withProperty(BlockDepthsPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
