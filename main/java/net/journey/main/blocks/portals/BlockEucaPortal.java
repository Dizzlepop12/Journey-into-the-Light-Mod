...
public class BlockEucaPortal extends BlockBreakable {
	...
	public static Block portal = JourneyBlocks.eucaPortal;
	public static Block portalFrame = JourneyBlocks.eucaPortalFrame;
	public static Block fire = JourneyBlocks.essenceFire;
	public int dimensionID = Config.euca;
	
	public BlockEucaPortal(String name, String finalName) {
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		BlockEucaPortal.Size blockportal$size = new BlockEucaPortal.Size(worldIn, pos, EnumFacing.Axis.X);
		if(blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			blockportal$size.placePortalBlocks();
			return true;
		} else {
			BlockEucaPortal.Size blockportal$size1 = new BlockEucaPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
		...
		BlockEucaPortal.Size blockportal$size = new BlockEucaPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
		LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

		if(!blockportal$size.isValid()) {
			enumfacing$axis = EnumFacing.Axis.X;
			blockportal$size = new BlockEucaPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
		}
		...
		public void placePortalBlocks() {
			for(int i = 0; i < this.width; i++) {
				...
				for(int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), portal.getDefaultState().withProperty(BlockEucaPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
