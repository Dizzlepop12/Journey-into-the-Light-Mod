...
public class BlockFrozenPortal extends BlockBreakable {
	...
	public static Block portal = JourneyBlocks.frozenPortal;
	public static Block portalFrame = JourneyBlocks.frozenPortalFrame;
	public static Block fire = JourneyBlocks.essenceFire;
	public int dimensionID = Config.frozen;
	
	public BlockFrozenPortal(String name, String finalName) {
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		BlockFrozenPortal.Size blockportal$size = new BlockFrozenPortal.Size(worldIn, pos, EnumFacing.Axis.X);
		if(blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			blockportal$size.placePortalBlocks();
			return true;
		} else {
			BlockFrozenPortal.Size blockportal$size1 = new BlockFrozenPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
		...
		BlockFrozenPortal.Size blockportal$size = new BlockFrozenPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
		LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

		if(!blockportal$size.isValid()) {
			enumfacing$axis = EnumFacing.Axis.X;
			blockportal$size = new BlockFrozenPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
		}
		...
		public void placePortalBlocks() {
			for(int i = 0; i < this.width; i++) {
				...
				for(int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), portal.getDefaultState().withProperty(BlockFrozenPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
