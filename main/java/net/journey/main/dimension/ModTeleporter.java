package net.journey.main.dimension;
...
public class ModTeleporter extends Teleporter {
    ...
    private final Long2ObjectMap<ModTeleporter.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap(4096);
	...
	public boolean placeInExistingPortal(Entity e, float par2) {
		...
		if (d0 >= 0.0D)
		{
			if (flag1)
			{
                this.destinationCoordinateCache.put(k, new ModTeleporter.PortalPosition(object, this.worldServerInstance.getTotalWorldTime()));
			}
								...
								for (i4 = 0; i4 < 3; ++i4) {
									for (j4 = 0; j4 < 4; ++j4) {
										for (k4 = -1; k4 < 4; ++k4) {
											...
											if (k4 < 0 && !this.worldServerInstance.getBlockState(new BlockPos(l4, i5, j5)).getBlock().getMaterial(this.worldServerInstance.getBlockState(new BlockPos(l4, i5, j5)).getBlock().getDefaultState()).isSolid() || k4 >= 0 && !this.worldServerInstance.isAirBlock(new BlockPos(l4, i5, j5)))
												continue label274;
										}
									}
								}
								...
								for (j3 = l1; j3 < l1 + 2; ++j3) {
									...
									for (i4 = 0; i4 < 4; ++i4) {
										for (j4 = -1; j4 < 4; ++j4) {
											...
											if (j4 < 0 && !this.worldServerInstance.getBlockState(new BlockPos(k4, l4, i5)).getBlock().getMaterial(this.worldServerInstance.getBlockState(new BlockPos(k4, l4, i5)).getBlock().getDefaultState()).isSolid() || j4 >= 0 && !this.worldServerInstance.isAirBlock(new BlockPos(k4, l4, i5)))
												continue label222;
										}
									}
