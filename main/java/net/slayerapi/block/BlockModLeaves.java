...
public class BlockModLeaves extends BlockMod implements IShearable {
                    ...
                    for (l1 = -b0; l1 <= b0; ++l1) {
                        for (i2 = -b0; i2 <= b0; ++i2) {
                            for (j2 = -b0; j2 <= b0; ++j2) {
                                ...
                                if (!block.canSustainLeaves(s, world, new BlockPos(x + l1, y + i2, z + j2))) {
                                    if (block.isLeaves(s, world, new BlockPos(x + l1, y + i2, z + j2))) this.adjacentTreeBlocks[(l1 + k1) * j1 + (i2 + k1) * b1 + j2 + k1] = -2;
    ...
    @Override
    @SideOnly(Side.CLIENT)
    public boolean shouldSideBeRendered(IBlockState state, IBlockAccess w, BlockPos pos, EnumFacing s) {
        return true;
    }
