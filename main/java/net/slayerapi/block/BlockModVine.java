...
public class BlockModVine extends BlockMod implements net.minecraftforge.common.IShearable {
	...
	public BlockModVine(String name, String finalName) {
		super(EnumMaterialTypes.VINES, name, finalName, JourneyTabs.blocks);
			...
			if (worldIn.rand.nextInt(4) == 0) {
				...
				if (enumfacing1 == EnumFacing.UP && pos.getY() < 255 && worldIn.isAirBlock(blockpos2)) {
					...
					if (!flag) {
						...
						if (block1.getMaterial(getDefaultState()) == Material.AIR) {
						...
						else if (block1.getMaterial(getDefaultState()).isOpaque() && iblockstate3.isFullCube()) {
					...
					if (pos.getY() > 1) {
						...
						if (block.getMaterial(getDefaultState()) == Material.AIR) {
